class Group:
    ...
    def __repr__(self):
        out = []
        out.append('{}({}): '.format(self.team, self.name))
        out.append(self.n)
    ...
    def clone(self):
        c = Group(self.name, self.team, self.n, self.hp, self.d, self.dt, self.i)
        for t in self.immune:
            c.immune.append(t)
        for t in self.weak:
            c.weak.append(t)
        return c
...
def simulate(groups):
    ...
    while len_team_immune() > 0 and len_team_infection() > 0:
        ...
        while len(q) > 0:
            ...
            damage_total += target.get_attacked(attacker)
        if damage_total == 0:
            return sum([g.n for g in groups]), None
        groups = filter(lambda g: g.is_alive(), groups)
    return sum([g.n for g in groups]), groups[0].team
...
def solve(groups):
    ...
    while team != 'Immune':
        ...
        for g in groups:
            clone = g.clone()
            if clone.team == 'Immune':
                clone.d += boost
            boosted_clones.append(clone)
        remaining, team = simulate(boosted_clones)
        print boost, remaining, team
        boost += 1
